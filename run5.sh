list=(25.0 33.0 50.0)
spinlist=(0.0 1.0 0.1 0.25 0.5 2.0)
dir=$(pwd)
echo "$dir"
#rm -rf inter_result.txt
for ((j = 1; j < ${#spinlist[@]}; j++))
do
for ((i = 0; i < ${#list[@]}; i++)) 
    do
        echo "Start process"
	...
	echo "Running sdpb"
        docker run --shm-size=4096m -v "$dir":/data bootstrapcollaboration/sdpb:master mpirun --allow-run-as-root -n 32 sdpb --precision=1024 --procsPerNode=32 --maxIterations=5000 -s /data/out
	
	touch spin0/spin0_result_"$j".txt
        
	python3 store_result.py out_out/out.txt spin0/spin0_result_"$j".txt
