...
new[k_,q_,x_,l_]:=FullSimplify[1/((k-q-1)!)*1/((q)!)(D[D[(s+t-8/3*M^2)(1/(x-s+1)+1/(x+s+t+1-4*M^2))*1/((x-4/3*M^2+1)(x+t-8/3*M^2+1))*LegendreP[l,1+(2t)/(x+1-4*M^2)],{t,q}]/.{t -> 4/3*M^2},{s,k-q-1}]/.{s -> 4/3*M^2})];
(*new[k_,q_,x_,l_]:=FullSimplify[1/((k-q-1)!)*1/((q)!)(D[D[(s+t)(1/(x-s+1)+1/(x+s+t+1))*1/((x+1)(x+t+1))*LegendreP[l,1+(2t)/(x+1)],{t,q}]/.{t -> 0},{s,k-q-1}]/.{s -> 0})];*)
...
matA[J_, f_]:={{{-new[3,1,f,J],new[2,0,f,J]/2,new[5,2,f,J]-new[5,3,f,J],new[6,2,f,J]-new[6,4,f,J],3*new[4,0,f,J]-new[4,2,f,J],new[7,2,f,J]-new[7,5,f,J],new[7,3,f,J]-new[7,4,f,J],new[8,2,f,J]-new[8,6,f,J],new[8,3,f,J]-new[8,5,f,J]}}};
...
SDP[datfile_] := Module[

    {
        ...
        *)
		...
		norm = {bound, 0, 0, 0, 0, 0, 0, 0, 0},
		obj = {0, -1, 0, 0, 0, 0, 0, 0, 0}
